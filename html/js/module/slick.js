...
export const myslick = {
  init() {
    const __ = this;
    const $for = $('.products-slider-for');
    const $nav = $('.products-slider-nav');

    const $progressBar = $('.slick-progress');
    const $progressBarLabel = $progressBar.find('span');

    let firstslidecalc = (1 / $nav.children().length) * 100;
    $progressBarLabel.css({
      width: `${firstslidecalc}%`,
    });

    $(window).on('resize load', function () {
      $progressBarLabel.css({ width: `${firstslidecalc}%` });
    });
